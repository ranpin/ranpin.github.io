...
const DataValidation: React.FC<DataValidationProps> = ({ _onValidate }) => {
  ...
  const performValidation = async () => {
    ...
    try {
      ...
      publications.forEach((pub, index) => {
        validateSection(
          `论文 #${index + 1}: ${pub.title || '未命名'}`,
          pub,
          validationRules.publications,
          results,
        );
      });
      ...
      internships.forEach((internship, index) => {
        validateSection(
          `工作经历 #${index + 1}: ${internship.position || '未命名'}`,
          internship,
          validationRules.internships,
          results,
        );
      });

      // 验证荣誉奖项
      const honors = JSON.parse(
        localStorage.getItem('portfolio_honors') || '[]',
      );
      honors.forEach((honor, index) => {
        validateSection(
          `荣誉奖项 #${index + 1}: ${honor.award || '未命名'}`,
          honor,
          validationRules.honors,
          results,
        );
      });

      // 验证博客
      const academicBlogs = JSON.parse(
        localStorage.getItem('portfolio_academic_blogs') || '[]',
      );
      const engineeringBlogs = JSON.parse(
        localStorage.getItem('portfolio_engineering_blogs') || '[]',
      );
      [...academicBlogs, ...engineeringBlogs].forEach((blog, index) => {
        validateSection(
          `博客 #${index + 1}: ${blog.title || '未命名'}`,
          blog,
          validationRules.blogs,
          results,
        );
      });
  ...
  const calculateCompletionRate = (results) => {
    ...
    const passedChecks =
      100 - results.errors.length - results.warnings.length * 0.5;
