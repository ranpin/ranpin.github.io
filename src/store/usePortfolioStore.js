...
export const usePortfolioStore = create(
  persist(
    (set, get) => ({
      ...
      setInternships: (data) => set({ internships: sanitizeData(data, 'internship') }),
      ...
      deleteTab: (key) => set((state) => {
        const newOrder = state.resumeTabOrder.filter(k => k !== key);
        const newCategory = state.resumeCategory === key && newOrder.length > 0 
          ? newOrder[0] 
      ...
      // Actions - 通用 CRUD 操作
      updateItem: (collection, id, data) => set((state) => {
        const items = state[collection];
        if (!Array.isArray(items)) return {};
        return {
          [collection]: items.map(item => item.id === id ? { ...item, ...data } : item)
        };
      }),
      ...
      addItem: (collection, newItem) => set((state) => {
        const items = state[collection];
        if (!Array.isArray(items)) return {};
        return {
      ...
      updateItemAt: (collection, index, data) => set((state) => {
        const items = state[collection];
      ...
      deleteItemAt: (collection, index) => set((state) => {
        const items = state[collection];
        if (!Array.isArray(items) || index < 0 || index >= items.length) return {};
      ...
      addItemAt: (collection, index, newItem) => set((state) => {
        const items = state[collection];
        if (!Array.isArray(items)) return {};
        const newItems = [...items];
