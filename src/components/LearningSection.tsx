...
interface LearningSectionProps {
  ...
  handleDeleteWithUndo: (items: unknown[], type: string) => void;
  handleInsertAt?: (type: string, index: number) => void;
  handleBlogClick?: (blog: unknown) => void;
}
...
const LearningSection: React.FC<LearningSectionProps> = ({
    ...
    return (
      <div className="space-y-4">
        <AnimatePresence mode="popLayout">
          {filter.data.map((blog: unknown, index: number) => (
            <motion.div
              key={blog.id || index}
            ...
            >
              <EditableCard
                ...
                onInsertBefore={() =>
                  handleInsertAt && handleInsertAt(`blog-${category}`, index)
                }
                onInsertAfter={() =>
                  handleInsertAt &&
                  handleInsertAt(`blog-${category}`, index + 1)
                }
                className={`border-l-4 ${borderColor} pl-6 py-4 hover:bg-gray-50 transition-colors cursor-pointer`}
              >
                <div onClick={() => handleBlogClick && handleBlogClick(blog)}>
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-1">
                        <h3
                          className={`text-lg font-medium text-gray-800 ${hoverColor} transition-colors`}
                        >
                          {blog.title || '未命名博客'}
                        </h3>
                        {blog.category && (
                    ...
                    <i
                      className={`fas fa-chevron-right text-gray-300 ${iconColor.replace('text-', 'hover:text-')} transition-colors text-sm`}
                    ></i>
                  </div>
          ...
          >
            ...
            {isAdminMode && (
              <p className="text-sm mt-2">
                点击任意位置插入点添加{isAcademic ? '学术' : '工程'}博客
              </p>
            )}
          </motion.div>
        )}
  ...
  return (
    ...
    >
      ...
      {/* 学术研究记录 */}
      {learningCategory === 'academic' &&
        renderBlogList(academicFilter, 'academic')}

      {/* 工程技术记录 */}
      {learningCategory === 'engineering' &&
        renderBlogList(engineeringFilter, 'engineering')}
    </motion.div>
  );
};
