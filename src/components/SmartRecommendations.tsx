      ...
      if (currentItem.tags && Array.isArray(currentItem.tags)) {
        Object.entries(allContent).forEach(([type, items]) => {
          ...
          items.forEach((item) => {
            ...
            // 时间相关性（同年份的内容）
            const currentYear = extractYear(
              currentItem.period || currentItem.year || currentItem.date,
            );
            const itemYear = extractYear(item.period || item.year || item.date);
            if (
              currentYear &&
              itemYear &&
              Math.abs(currentYear - itemYear) <= 1
            ) {
              score += 5;
            }
            ...
            const itemText = (
              item.title +
              ' ' +
              (item.description || item.abstract || item.summary || '')
            ).toLowerCase();
          ...
          return (
            ...
            >
              <div className="flex items-start space-x-4 mb-4">
                <div
                  className={`w-12 h-12 ${typeInfo.bg} rounded-xl flex items-center justify-center flex-shrink-0`}
                >
                  <i
                    className={`${typeInfo.icon} ${typeInfo.color} text-lg`}
                  ></i>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2 mb-1">
                    <span
                      className={`px-2 py-1 ${typeInfo.bg} ${typeInfo.color} rounded text-xs font-medium`}
                    >
                      {typeInfo.name}
                    </span>
