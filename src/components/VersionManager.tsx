...
import { VersionManagerProps } from '../types';
...
const VersionManager: React.FC<VersionManagerProps> = ({
  ...
  const [selectedVersion, setSelectedVersion] = useState<unknown>(null);
    ...
    try {
      const currentData = {
        personalInfo: JSON.parse(
          localStorage.getItem('portfolio_personal_info') || '{}',
        ),
        recentNews: JSON.parse(
          localStorage.getItem('portfolio_recent_news') || '[]',
        ),
        projects: JSON.parse(
          localStorage.getItem('portfolio_projects') || '[]',
        ),
        publications: JSON.parse(
          localStorage.getItem('portfolio_publications') || '[]',
        ),
        internships: JSON.parse(
          localStorage.getItem('portfolio_internships') || '[]',
        ),
        honors: JSON.parse(localStorage.getItem('portfolio_honors') || '[]'),
        academicBlogs: JSON.parse(
          localStorage.getItem('portfolio_academic_blogs') || '[]',
        ),
        engineeringBlogs: JSON.parse(
          localStorage.getItem('portfolio_engineering_blogs') || '[]',
        ),
      };
      ...
      const existingVersions = JSON.parse(
        localStorage.getItem('portfolio_versions') || '[]',
      );
  ...
  const restoreFullVersion = (version) => {
    if (
      window.confirm(
        `确定要完整恢复到版本 "${version.description}" 吗？当前数据将被覆盖！`,
      )
    ) {
      try {
        // 恢复数据到localStorage
  ...
  const executeSelectiveRestore = () => {
    ...
    const selectedModules = Object.keys(restoreSelection).filter(
    ...
    if (
      ...
      try {
        ...
        selectedModules.forEach((key) => {
          ...
          localStorage.setItem(
            storageKey,
            JSON.stringify(restoreVersion.data[key]),
          );
        });
  ...
  const toggleSelectAll = () => {
    const allSelected = Object.values(restoreSelection).every(
      (selected) => selected,
    );
    const newSelection = {};
          ...
          if (versionCount !== currentCount) {
            ...
            if (diff > 0) {
              ...
              changes.details.push(
                `${getModuleName(key)}: 删除 ${Math.abs(diff)} 项`,
              );
            }
          }
        }
      ...
      if (key === 'personalInfo') {
        ...
        details.push({
          ...
          items: [
            ...
            info.researchInterests &&
              `研究兴趣: ${Array.isArray(info.researchInterests) ? info.researchInterests.join(', ') : info.researchInterests}`,
            info.socialLinks &&
              `社交链接: ${Object.keys(info.socialLinks).length} 个`,
          ].filter(Boolean),
        });
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="bg-blue-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-blue-600">
                          {selectedVersion.data.projects?.length || 0}
                        </div>
                        <div className="text-blue-800">项目经历</div>
                      </div>
                      <div className="bg-green-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-green-600">
                          {selectedVersion.data.publications?.length || 0}
                        </div>
                        <div className="text-green-800">学术论文</div>
                      </div>
                      <div className="bg-purple-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-purple-600">
                          {selectedVersion.data.internships?.length || 0}
                        </div>
                        <div className="text-purple-800">工作经历</div>
                      </div>
                      <div className="bg-orange-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-orange-600">
                          {selectedVersion.data.honors?.length || 0}
                        </div>
                        <div className="text-orange-800">荣誉奖项</div>
                      </div>
                      <div className="bg-indigo-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-indigo-600">
                          {selectedVersion.data.recentNews?.length || 0}
                        </div>
                        <div className="text-indigo-800">最新动态</div>
                      </div>
                      <div className="bg-pink-50 rounded-lg p-3 text-center">
                        <div className="text-xl font-bold text-pink-600">
                          {(selectedVersion.data.academicBlogs?.length || 0) +
                            (selectedVersion.data.engineeringBlogs?.length ||
                              0)}
                        </div>
                  ...
                  <div className="bg-white rounded-lg border p-4">
                    ...
                    <div className="space-y-4 max-h-80 overflow-y-auto">
                      {getBackupDetails(selectedVersion.data).map(
                        (detail, index) => (
                          <div
                            key={index}
                        ...
                        <div className="bg-white rounded-lg border p-4">
                          ...
                          <div className="space-y-2">
                            {changes.details.map((detail, index) => (
                              <div
                                key={index}
                                className="text-sm text-gray-600 bg-yellow-50 rounded px-3 py-2"
                              >
                                {detail}
                              </div>
                            ))}
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                  ...
                  <div className="bg-purple-50 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
                    ...
                    <div className="text-xs sm:text-sm text-purple-700 space-y-1">
                      <p>
                        <strong>版本：</strong>
                        {restoreVersion.description}
                      </p>
                      <p>
                        <strong>时间：</strong>
                        {new Date(restoreVersion.timestamp).toLocaleString(
                          'zh-CN',
                        )}
                      </p>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 p-3 bg-gray-50 rounded-lg space-y-2 sm:space-y-0">
                    <label className="flex items-center space-x-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={Object.values(restoreSelection).every(
                          (selected) => selected,
                        )}
                    ...
                    <span className="text-xs sm:text-sm text-gray-500">
                      已选择{' '}
                      {Object.values(restoreSelection).filter(Boolean).length} /{' '}
                      {Object.keys(restoreSelection).length} 个模块
                    </span>
                  </div>

                  {/* 模块选择列表 */}
                  <div className="space-y-2 sm:space-y-3">
                    {Object.entries(restoreVersion.data).map(
                          ...
                          >
                            <div className="flex items-start space-x-2 sm:space-x-3">
                              <input
                                ...
                                checked={isSelected}
                                onChange={() =>
                                  toggleModuleSelection(moduleKey)
                                }
