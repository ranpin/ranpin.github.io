...
const ProjectEditor = ({ formData, onChange }) => {
  ...
  const inputClass =
    'w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 focus:bg-white transition-all text-sm';
  const labelClass =
    'block text-xs font-bold text-gray-700 mb-1.5 flex items-center justify-between';
  const sectionClass =
    'mb-6 p-4 bg-white rounded-xl shadow-sm border border-gray-100';
  ...
  const getFieldDefaultTitle = (field) => {
    const titles = {
      ...
      yourRole: '你的角色',
      architectureDetail: '技术架构详解',
      technicalChallenges: '核心技术难点',
      results: '实验结果/主要成果',
      achievements: '主要成就',
      interviewHighlights: '面试亮点',
      discussionTopics: '可延伸讨论的话题',
    };
      ...
      <div className="w-3/5 h-full overflow-y-auto custom-scrollbar pr-2">
        ...
        <div className={sectionClass}>
          <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center">
            <i className="fas fa-info-circle mr-2 text-blue-500"></i> 基础信息
          </h4>
        ...
        <div className={sectionClass}>
          <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center">
            <i className="fas fa-layer-group mr-2 text-indigo-500"></i> 深度详情
          </h4>

          {['businessContext', 'yourRole', 'architectureDetail'].map(
            (field) => (
              <div key={field} className="mb-4 last:mb-0">
                <CustomTitleInput
                  field={field}
                  defaultTitle={getFieldDefaultTitle(field)}
                />
                <textarea
                  rows={4}
                  ...
                  placeholder={`请输入${getTitle(field, getFieldDefaultTitle(field))}...`}
                />
              </div>
            ),
          )}
          ...
          {[
            ...
            <div key={field} className="mb-4 last:mb-0">
              ...
              <textarea
                ...
                className={`${inputClass} font-mono text-xs`}
                placeholder={`每行一条${getTitle(field, getFieldDefaultTitle(field))}...`}
              />
            </div>
          ))}
        </div>
        ...
        <div className={sectionClass}>
          <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center">
            <i className="fas fa-photo-video mr-2 text-purple-500"></i>{' '}
            多媒体与链接
          </h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>演示图片 URL</label>
              <input
                type="text"
                value={formData.demoImage || ''}
                onChange={(e) => handleChange('demoImage', e.target.value)}
                className={inputClass}
              />
            </div>
            <div>
              <label className={labelClass}>架构图 URL</label>
              <input
                type="text"
                value={formData.architectureImage || ''}
                onChange={(e) =>
                  handleChange('architectureImage', e.target.value)
                }
                className={inputClass}
              />
            </div>
            ...
            <div>
              <label className={labelClass}>在线演示 URL</label>
              <input
                type="text"
