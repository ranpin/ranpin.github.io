  ...
  const searchContent = (searchQuery) => {
    ...
    try {
      ...
      internships.forEach((internship) => {
        if (
          internship.position?.toLowerCase().includes(searchText) ||
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <i
            className={`fas ${isSearching ? 'fa-spinner fa-spin' : 'fa-search'} text-gray-400`}
          ></i>
        </div>
        <input
          type="text"
              ...
              {searchResults.map((result, index) => (
                ...
                >
                  <div className="flex items-start space-x-3">
                    <div
                      className={`flex-shrink-0 w-6 h-6 md:w-8 md:h-8 rounded-full bg-gray-100 flex items-center justify-center ${result.color}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center mt-1 md:mt-2">
                        <span
                          className={`text-xs px-2 py-1 rounded-full bg-gray-100 ${result.color}`}
                        >
                          {result.type === 'project'
                            ? '项目'
                            : result.type === 'publication'
                              ? '论文'
                              : result.type === 'academic-blog'
                                ? '学术'
                                : result.type === 'engineering-blog'
                                  ? '工程'
